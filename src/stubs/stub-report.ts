 ...
 *
 * `name` must match the `name` of the entry in build-scripts/stubs.cjs.
 */
export const reportStubbed = (name: string, detail: string): void => {
  // eslint-disable-next-line no-console
  console.error(
    `[Velbus] "${name}" is stubbed out in this build, so ${detail} does not work here. ` +
      "The Velbus panel is not supposed to need it — if it does, remove the entry from " +
