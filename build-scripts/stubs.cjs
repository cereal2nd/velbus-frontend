/* Modules the Velbus panel replaces with a stub at build time.
 *
 * The panel is built from the full homeassistant-frontend source tree, so it inherits every
 * dependency HA has — camera players, map renderers, chart engines — whether or not the Velbus
 * panel can reach them. Unreachable code still ships: rspack emits it as lazy chunks that no
 * Velbus user ever downloads, but `velbus_frontend/dist` is zipped into the wheel as-is, so
 * every Home Assistant install pays for it on disk. Replacing the module that pulls a library
 * in drops the whole subtree behind it.
 *
 ...
 * reached:
 *
 *     [Velbus] "leaflet maps" is stubbed out in this build ...
 *
 ...
 * merely moved: `grep -rl <marker> velbus_frontend/dist` should come back empty.
 */
